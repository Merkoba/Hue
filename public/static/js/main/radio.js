...
Hue.setup_radio = function (mode, odata = {}) {
  ...
  if (mode === "restart") {
    ...
    data.message = `${
      data.setter
    } changed the radio to: ${Hue.utilz.conditional_quotes(data.title)}`

    if (data.type === "audio") {
      if (data.source.slice(-1) === "/") {
        data.metadata_url = `${data.source
          .slice(0, -1)
          .split("/")
          .slice(0, -1)
          .join("/")}/status-json.xsl`
      } else {
        data.metadata_url = `${data.source
          .split("/")
          .slice(0, -1)
...
Hue.push_radio_changed = function (data) {
  ...
  if (Hue.radio_changed.length > Hue.config.media_changed_crop_limit) {
    Hue.radio_changed = Hue.radio_changed.slice(
      Hue.radio_changed.length - Hue.config.media_changed_crop_limit
    )
  }
...
Hue.load_radio = function (force = false) {
  ...
  if (item.type === "audio") {
    if ($("#audio_player").length === 0) {
      $("#media_audio_audio_container").html(
    ...
    if (Hue.youtube_player) {
      ...
      if (id[0] === "video") {
        if (Hue.radio_started) {
          Hue.youtube_player.loadVideoById({
            videoId: id[1],
      ...
      Hue.youtube_player.setVolume(
        Hue.get_nice_volume(Hue.room_state.radio_volume)
      )
    }
    ...
    if (Hue.soundcloud_player) {
      Hue.soundcloud_player.load(item.source, {
        ...
        callback: function () {
          ...
          Hue.soundcloud_player.setVolume(
            Hue.get_nice_volume(Hue.room_state.radio_volume)
          )
        },
      })
        ...
        try {
          ...
          if (Array.isArray(data.icestats.source)) {
            for (let i = 0; i < data.icestats.source.length; i++) {
              ...
              if (
                source.listenurl.includes(
                  Hue.loaded_radio.source.split("/").pop()
                )
              ) {
            ...
            data.icestats.source.listenurl.includes(
              Hue.loaded_radio.source.split("/").pop()
            )
          ) {
          ...
          if (
            !source ||
            (source.artist === undefined && source.title === undefined)
          ) {
            Hue.on_radio_get_metadata_error()
            return false
          }
...
Hue.announce_room_radio_mode_change = function (data) {
  Hue.show_room_notification(
    data.username,
    `${data.username} changed the radio mode to ${data.what}`
  )
...
Hue.setup_radio_widget = function () {
  ...
  $("#toggle_radio_state").on("mouseenter", function (e) {
    ...
    if (Hue.radio_started_date) {
      title = `Started: ${Hue.utilz.capitalize_words(
        timeago.format(Hue.radio_started_date)
      )}`
    } else {
      title = ""
    }
