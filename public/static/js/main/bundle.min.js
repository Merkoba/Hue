...
Hue.setup_message_board=function(){$("#message_board_post_icon").click(function(){Hue.create_message_board_post()}),$("#message_board_post_button").click(function(){Hue.submit_message_board_post()}),$("#message_board_post_textarea").on("input blur",function(){let e=$(this).val().substring(0,Hue.config.max_message_board_post_length);$(this).val(e)})},Hue.add_post_to_message_board=function(e){let s=$("\n    <div class='message_board_item modal_item'>\n        <div class='message_board_text dynamic_title'></div>\n    </div>"),a=s.find(".message_board_text").eq(0);a.text(e.message).urlize();let _=Hue.utilz.nice_date(e.date);a.attr("title",_),a.data("date",e.date),a.data("otitle",_),$("#message_board_container").prepend(s),$("#message_board_container").find(".message_board_item").length>Hue.config.max_message_board_posts&&$("#message_board_container").find(".message_board_item").last().remove()},Hue.init_message_board=function(e){for(let s of e.message_board_posts)Hue.add_post_to_message_board(s);Hue.last_message_board_post_date=e.last_message_board_post_date,Hue.check_message_board_delay(),Hue.check_last_message_board_post()},Hue.create_message_board_post=function(){if(!Hue.message_board_posting_enabled){let e=Math.round(Hue.config.message_board_post_delay/60/1e3),s=1===e?"minute":"minutes";return Hue.msg_info.show(`You can post in the message board once every ${e} ${s}`),!1}Hue.msg_message_board_post.show(function(){$("#message_board_post_textarea").focus()})},Hue.show_message_board=function(){Hue.msg_message_board.show(),Hue.check_last_message_board_post()},Hue.submit_message_board_post=function(){let e=Hue.utilz.clean_string2($("#message_board_post_textarea").val());if(!e||e.length>Hue.config.max_message_board_post_length)return!1;Hue.msg_message_board_post.close(),Hue.socket_emit("message_board_post",{message:e})},Hue.on_message_board_received=function(e){Hue.add_post_to_message_board(e),Hue.check_last_message_board_post()},Hue.last_message_board_post_date_update=function(e){Hue.last_message_board_post_date=e.date,Hue.check_message_board_delay()},Hue.check_message_board_delay=function(){let e=Date.now()-Hue.last_message_board_post_date;clearTimeout(Hue.message_board_post_delay_timeout),e<Hue.config.message_board_post_delay?($("#message_board_post_icon").css("opacity",.5),Hue.message_board_post_delay_timeout=setTimeout(function(){Hue.check_message_board_delay()},e+1e3),Hue.message_board_posting_enabled=!1):($("#message_board_post_icon").css("opacity",1),Hue.message_board_posting_enabled=!0)},Hue.check_last_message_board_post=function(){let e=Hue.get_local_storage(Hue.ls_last_message_board_post_checked);e||(e=Date.now(),Hue.save_local_storage(Hue.ls_last_message_board_post_checked,e));let s=$("#message_board_container").find(".message_board_item").first().find(".message_board_text").eq(0);s.data("date")>e?(Hue.msg_message_board.is_open()||$("#topbox_message_board_label").text("New Messages"),Hue.save_local_storage(Hue.ls_last_message_board_post_checked,s.data("date"))):$("#topbox_message_board_label").text("")};
