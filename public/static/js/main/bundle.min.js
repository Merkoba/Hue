...
Hue.setup_message_board=function(){$("#message_board_post_icon").click(function(){Hue.create_message_board_post()}),$("#message_board_post_button").click(function(){Hue.submit_message_board_post()}),$("#message_board_post_textarea").on("input blur",function(){let e=$(this).val().substring(0,Hue.config.max_message_board_post_length);$(this).val(e)}),$("#message_board_container").on("click",".message_board_delete",function(){if(confirm("Are you sure?")){let e=$(this).closest(".message_board_item").data("id");e&&Hue.socket_emit("delete_message_board_post",{id:e})}}),$("#message_board_post_textarea").on("focus",function(){Hue.writing_message_board_post=!0}),$("#message_board_post_textarea").on("blur",function(){Hue.writing_message_board_post=!1});let e=Math.round(Hue.config.message_board_post_delay/60/1e3),a=`You can post once every ${e} ${1===e?"minute":"minutes"}`;$("#message_board_post_textarea").attr("placeholder",a),Hue.get_last_message_board_post_checked()},Hue.add_post_to_message_board=function(e){let a=$("\n    <div class='message_board_item modal_item'>\n        <div class='message_board_text dynamic_title'></div>\n        <div><div class='message_board_delete action pointer inline'>Delete</div></div>\n    </div>");a.data("id",e.id);let s=a.find(".message_board_text").eq(0);s.text(e.message).urlize();let _=Hue.utilz.nice_date(e.date);s.attr("title",_),s.data("date",e.date),s.data("otitle",_),0===$("#message_board_container .message_board_item").length?$("#message_board_container").html(a):$("#message_board_container").prepend(a),$("#message_board_container").find(".message_board_item").length>Hue.config.max_message_board_posts&&$("#message_board_container").find(".message_board_item").last().remove()},Hue.init_message_board=function(e){for(let a of e.message_board_posts)Hue.add_post_to_message_board(a);Hue.last_message_board_post_date=e.last_message_board_post_date,Hue.check_message_board_delay(),Hue.check_last_message_board_post()},Hue.show_message_board=function(){Hue.msg_message_board.show(function(){Hue.update_message_board_date(),Hue.check_last_message_board_post()})},Hue.submit_message_board_post=function(){let e=Hue.utilz.clean_string2($("#message_board_post_textarea").val());if(!e||e.length>Hue.config.max_message_board_post_length)return!1;$("#message_board_post_textarea").val(""),Hue.socket_emit("message_board_post",{message:e})},Hue.on_message_board_received=function(e){Hue.add_post_to_message_board(e),Hue.check_last_message_board_post()},Hue.last_message_board_post_date_update=function(e){Hue.last_message_board_post_date=e.date,Hue.check_message_board_delay()},Hue.check_message_board_delay=function(){let e=Date.now()-Hue.last_message_board_post_date;clearTimeout(Hue.message_board_post_delay_timeout),e<Hue.config.message_board_post_delay?($("#message_board_post_textarea").css("display","none"),Hue.message_board_post_delay_timeout=setTimeout(function(){Hue.check_message_board_delay()},e+1e3),Hue.message_board_posting_enabled=!1):($("#message_board_post_textarea").css("display","block"),Hue.message_board_posting_enabled=!0)},Hue.check_last_message_board_post=function(){$("#message_board_container").find(".message_board_item").first().find(".message_board_text").eq(0).data("date")>Hue.last_message_board_post_checked?Hue.msg_message_board.is_open()||$("#activity_left_message_board_label").text("New Posts"):$("#activity_left_message_board_label").text("")},Hue.update_message_board_date=function(){let e=$("#message_board_container").find(".message_board_item").first().find(".message_board_text").eq(0),a=e.data("date");a!==Hue.last_message_board_post_checked&&(Hue.last_message_board_post_checked=a,Hue.save_local_storage(Hue.ls_last_message_board_post_checked,e.data("date")))},Hue.check_message_board_permissions=function(){"admin"===Hue.role?$("#message_board_container").addClass("message_board_container_admin"):$("#message_board_container").removeClass("message_board_container_admin")},Hue.remove_message_board_post=function(e){$("#message_board_container").find(".message_board_item").each(function(){if($(this).data("id")===e.id)return $(this).remove(),!1})},Hue.get_last_message_board_post_checked=function(){Hue.last_message_board_post_checked=Hue.get_local_storage(Hue.ls_last_message_board_post_checked),Hue.last_message_board_post_checked||(Hue.last_message_board_post_checked=Date.now(),Hue.save_local_storage(Hue.ls_last_message_board_post_checked,Hue.last_message_board_post_checked))};
