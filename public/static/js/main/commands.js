...
Hue.execute_command = function (message, ans) {
  ...
  if (!command) {
    ...
    if (closest_command) {
      ...
      Hue.feedback(
        `Invalid command "${cmd}". Maybe it is missing an argument. 
        To start a message with ${Hue.config.commands_prefix} use ${Hue.config.commands_prefix}${Hue.config.commands_prefix}`
      )
      return ans
    }
  }
