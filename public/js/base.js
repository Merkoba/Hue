...
function start_socket()
{
	...
	socket.on('update', function(data) 
	{
		...
		if(data.type === 'announce_voice')
		{
			announce_voice(data);
		}
		if(data.type === 'announce_op')
		{
			announce_op(data);
		}
		if(data.type === 'announce_admin')
		{
			announce_admin(data);
		}
		if(data.type === 'announce_strip')
		{
			announce_strip(data);
		}
		...
		if(data.type === 'announce_unbanlast')
		{
			announce_unbanlast(data);
		}
		if(data.type === 'get_bannedcount')
		{
			if(data.count == 1)
			{
		...
		if(data.type === 'nothingtounban')
		{
			chat_announce('[', ']', "There was nothing to unban", 'small');
		}
		if(data.type === 'listbans')
		{
			show_listbans(data);
		}
		...
		if(data.type === 'announce_claim')
		{
			announce_claim(data);
		}
		if(data.type === 'made_private')
		{
			made_private(data);
		}
		if(data.type === 'made_public')
		{
			made_public(data);
		}
		if(data.type === 'changed_radiosrc')
		{
		...
		if(data.type === 'alreadyreserved')
		{
			chat_announce('[', ']', username + " is already reserved", 'small');
		}
		if(data.type === 'couldnotrecover')
		{
...
function replace_tabbed(word)
{
	...
	if(uname !== "")
	{
		...
		input.value = replaceBetween(s, tabbed_start, tabbed_end, uname + ' ').replace(/\s+/g, ' ');
...
function add_to_chat(msg)
{
	...
	if(document.hidden)
	{
		...
		{
			chat_area.append("<div class='dash_container'><hr class='dash'><div>&nbsp;</div></div>");
...
function chat_announce(brk1, brk2, msg, size, dotted=false)
{
	...
	{
		var fmsg = $("<div title='" + date + "' class='msg announcement announcement_" + size + "'>" + brk1 + " <span class='" + contclasses + "'></span><span class='dotted'></span> " + brk2 + "</div><div>&nbsp;</div>");
		$($(fmsg).find('.dotted').get(0)).text(dotted).urlize();
	}
	else
	{
		var fmsg = $("<div title='" + date + "' class='msg announcement announcement_" + size + "'>" + brk1 + " <span class='" + contclasses + "'></span> " + brk2 + "</div><div>&nbsp;</div>");
	}
...
function start_chat()
{
	$('#chat_area').append('<br><br><br><div class="clear">&nbsp;</div>');
	$('#input').focus();
...
function initial_volume()
{
	...
	{
		// Not calling set_volume because I don't want to save to localStorage needlesly
		var audio = $('#audio')[0];
		audio.volume = volume;
		$('#pup')[0].volume = volume;
		$('#volume').text(parseInt(Math.round((volume * 100))) + '%')
	}
}

function clear_chat()
{
	$('#chat_area').html('<br><br><br><div class="clear">&nbsp;</div>');
	show_intro();
	show_topic();
	show_mode();
	show_priv();
...
function set_mode(m)
{
	...
	else if(mode === 2)
	{
		...
		if(priv === 'admin' || priv === 'voice' || priv === 'op')
		{
			can_upload = true;
		}
	}
...
function made_private(data)
{
	...
	if(data.username === username)
	{
		chat_announce('~', '~', 'You made the room private', 'small');		
	}
	else
	{
		chat_announce('~', '~', data.username + ' made the room private', 'small');
	}
}
...
function made_public(data)
{
	...
	if(data.username === username)
	{
		chat_announce('~', '~', 'You made the room public', 'small');		
	}
	else
	{
		chat_announce('~', '~', data.username + ' made the room public', 'small');
	}
}
