module.exports = function(manager, vars, db, config, sconfig, utilz, logger)
{
    ...
    manager.update_user = function(_id, fields)
    {
        return new Promise((resolve, reject) =>
        {
            ...
            {
                vars.bcrypt.hash(fields.password, config.encryption_cost)
    ...
    manager.check_password = function(email, password, fields={})
    {
        return new Promise((resolve, reject) =>
        {
            ...
            .then(user =>
            {
                ...
                {
                    vars.bcrypt.compare(password, user.password)
    ...
    manager.save_message_board_date = function(user_id, room_id)
    {
        return new Promise((resolve, reject) =>
        {
            manager.get_user({_id:user_id}, {message_board_dates:1})
            ...
            .then(user =>
            {
                ...
                if(message_board_dates.length > config.max_message_board_dates_items)
                {
                    message_board_dates = message_board_dates.slice(0, config.max_message_board_dates_items)
                }

                manager.update_user(user_id, {message_board_dates:message_board_dates})
