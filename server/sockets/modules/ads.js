module.exports = function(handler, vars, io, db_manager, config, sconfig, utilz, logger)
{
    ...
    handler.attempt_show_image_ad = function(room_id, callback)
    {
        try
        {
            ...
            vars.fs.readdir(vars.path.join(vars.root_path, config.image_ads_path), function(err, files)
            {
                if(err)
                {
                    logger.log_error(err)
    ...
    handler.attempt_show_text_ad = function(room_id, callback)
    {
        try
        {
            vars.fs.readFile(vars.path.join(vars.root_path, config.text_ads_json_location), function(err, content)
            {
                if(err)
                {
                    logger.log_error(err)
