module.exports = function(handler, vars, io, db_manager, config, sconfig, utilz, logger)
{
    ...
    handler.public.change_tv_source = async function(socket, data)
    {
        ...
        if(vars.rooms[socket.hue_room_id].current_tv_source === data.src
        ...
        if(Date.now() - vars.rooms[socket.hue_room_id].last_tv_change < config.tv_change_cooldown)
        {
        ...
        if(utilz.is_url(data.src))
        {
            ...
            else if(data.src.includes("soundcloud.com"))
            {
                ...
                vars.soundcloud.get(`/resolve?url=${encodeURIComponent(data.src)}`, function(err, track)
                {
                    if(err)
                    {
                        handler.user_emit(socket, 'video_not_found', {})
